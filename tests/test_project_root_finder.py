...
def test_finds_project_root_hook(tmp_path: Path) -> None:
...
def test_fallback_to_git(tmp_path: Path) -> None:
...
def test_fallback_to_pipfile(tmp_path: Path) -> None:
...
def test_fallback_to_pyproject_toml(tmp_path: Path) -> None:
...
def test_custom_start_path_at_root(tmp_path: Path) -> None:
    """Test if starting directly at the project root."""
    create_file(tmp_path, '.project-root-hook')
    found = _get_project_root(start_path=tmp_path)
    assert found == tmp_path

def test_multiple_markers_prefers_project_hook(tmp_path: Path) -> None:
...
def test_fallback_order_is_respected(tmp_path: Path) -> None:
...
def test_deeply_nested_still_finds_root(tmp_path: Path) -> None:
